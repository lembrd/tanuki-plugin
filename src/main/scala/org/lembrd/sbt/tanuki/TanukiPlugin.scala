...
object TanukiPlugin extends AutoPlugin {
  ...
  def tanukiSettings = Seq(
    ...
    tanukiConf := Nil,
    appParameters := Nil,
    appParameters <+= (mainClass in Compile) map { _.getOrElse(sys.error("no mainClass in Compile specified")) },
...
//      "wrapper.app.parameter.1" -> mainClass.getOrElse(sys.error("no mainClass in Compile specified")),
    ...
    makeWrapperConf <<= (jvmParameters, appParameters, tanukiConf, appClasspath, target in Universal, mainClass in Compile) map { (jvmParams, appParams, conf, cp, t, mainClass) =>
      ...
      s"""
         ...
         |
         |""".stripMargin)
      .append( getWrapperString( jvmParams.zipWithIndex.map{
      ...
      .append( getWrapperString(appParams.zipWithIndex.map{
        case(p, idx) => s"wrapper.app.parameter.${idx+1}" -> p
      }) )
      .append(
      s"""
         |
